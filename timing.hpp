...
constexpr double kOutlierSigma = 3.0;

static double filtered_mean(const std::vector<float>& times_ms) {
    ...
    for (float t : times_ms) {
        if (std::abs(static_cast<double>(t) - mean) <= kOutlierSigma * stddev) {
